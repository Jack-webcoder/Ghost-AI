...
1. **Check CLERK_SECRET_KEY** — verify it is set:
   ...
   if [[ -n "${CLERK_SECRET_KEY:-}" ]]; then
     echo "CLERK_SECRET_KEY check passed."
   else
     echo "CLERK_SECRET_KEY check failed: variable is not set or empty." >&2
     exit 1
   fi
   ```
   If the check fails, stop and ask the user. Do not proceed without a valid key.
