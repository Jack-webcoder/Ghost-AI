...
- Check that no parent element is intercepting or blocking the drag events before they reach the
canvas
...
- The canvas background (dotted pattern) should be visible edge-to-edge underneath both sidebars - Sidebars should have a semi-transparent or solid background with a subtle shadow so they feel elevated above the canvas, not embedded in the layout
